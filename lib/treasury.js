...
// The viewer can't call api.democracycraft.net directly — the upstream doesn't send
// CORS headers, so a browser fetch is refused at preflight. We route through a
// stateless Cloudflare Worker (source in ../worker/) that forwards the request and
// adds Access-Control-Allow-Origin: *. The bearer token round-trips through the
// Worker but is never logged or stored.
export const DEFAULT_BASE = "https://dc-transaction-viewer-proxy.bukub.workers.dev";
